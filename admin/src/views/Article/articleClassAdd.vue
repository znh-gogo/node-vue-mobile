<template>
    <div class="userAdd">
        <h1 style="margin-top:0">{{id?'编辑':'添加'}}文章分类</h1>
...
export default {
    ...
    methods:{
       ...
       async save(){
           let res
            if(this.id){
                res= await this.$http.put(`rest/articleClass/${this.id}`,this.model)
                
            } else{
                res= await this.$http.post('rest/articleClass',this.model)
             }
        ...
        fetchData(){
            this.$http.get(`rest/articleClass/${this.id}`).then((res)=>{
                this.model=res.data
                })
        },
        // fetchRelativeData(){
