...
<div style="padding:20px 10px;background:#fff">
  ...
  <el-table
    ...
    <el-table-column
      ...
      width="150">
      <template slot-scope="scope">
        <el-button @click="$router.push(`/planEdit/${scope.row._id}`)" type="primary" size="small">编辑</el-button>
    ...
    methods: {
     async remove(row) {
          ...
          type: 'warning'
            ...
            let res = await this.$http.delete(ADMIN+`/rest/plan/${row._id}`)
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
      ...
      fetchGoods(){
          this.$http.get(ADMIN+'/rest/planmaker').then(()=>{
            this.$http.get(ADMIN+'/rest/plan'+`/${this.numPage}/${this.numSize}`).then((res)=>{
